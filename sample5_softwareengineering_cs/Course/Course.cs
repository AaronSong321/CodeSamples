...
namespace HIT.UES.Course
{
    public class Course : DatabaseType, IOrderedDataContainer<CourseChapter>
    {
        ...
        #region Query course
        public static List<Course> GetCourses(Predicate<Course> filter)
        {
            List<Course> ans = null;
            using (var a = new UESContext(Settings.databaseName))
            {
                ans = (from b in a.Courses where filter(b) select b).ToList();
            }
            return ans;
        }
        /*
        public List<CourseChapter> GetChapters()
        {
            if (Chapters == null)
            {
                Console.WriteLine($"Course {CourseName} has no chpaters in property {nameof(Chapters)}, loading from a temporary context.");
                using (var imaginar = new UESContext(Settings.databaseName))
                {
